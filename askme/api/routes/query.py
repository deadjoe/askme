...
class RetrievalDebugInfo(BaseModel):
    ...
    alpha: Optional[float] = Field(default=None, description="Alpha parameter for fusion")
    rrf_k: Optional[int] = Field(default=None, description="RRF k parameter")
    rerank_model: str = Field(..., description="Reranker model used")
    rerank_scores: Optional[List[float]] = Field(default=None, description="Reranking scores")
    latency_ms: int = Field(..., description="Total retrieval latency in milliseconds")
    embedding_latency_ms: int = Field(..., description="Embedding latency in milliseconds")
...
class QueryRequest(BaseModel):
    ...
    topk: int = Field(default=50, ge=1, le=100, description="Number of candidates to retrieve")
    alpha: float = Field(default=0.5, ge=0, le=1, description="Hybrid search alpha parameter")
    ...
    use_rag_fusion: bool = Field(default=False, description="Use RAG-Fusion multi-query")
    reranker: str = Field(default="bge_local", description="Reranker model: bge_local, cohere")
    max_passages: int = Field(default=8, ge=1, le=20, description="Maximum passages for generation")
...
class QueryResponse(BaseModel):
    ...
    retrieval_debug: Optional[RetrievalDebugInfo] = Field(default=None, description="Debug info")
