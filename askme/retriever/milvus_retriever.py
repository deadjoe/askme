...
try:
    ...
    logger.warning("Milvus hybrid search features not available. Using fallback implementation.")

from .base import VectorRetriever, Document, RetrievalResult, HybridSearchParams, SearchFusion
...
class MilvusRetriever(VectorRetriever):
    ...
    async def hybrid_search(
        ...
        try:
            # Build filter expression if provided
            expr = self._build_filter_expression(params.filters) if params.filters else None
    ...
    async def _hybrid_search_rrf(
        ...
        if HYBRID_SEARCH_AVAILABLE:
            try:
                ...
                logger.debug(f"Native RRF hybrid search returned {len(retrieval_results)} results")
                return retrieval_results
                
            except Exception as e:
                logger.warning(f"Native hybrid search failed, falling back to manual RRF: {e}")
    ...
    async def _hybrid_search_rrf_fallback(
        ...
        dense_results = await self.dense_search(query_embedding, params.topk, params.filters)
        sparse_results = await self.sparse_search(query_terms, params.topk, params.filters)
        ...
        logger.debug(f"Fallback RRF hybrid search returned {len(fused_results)} results")
