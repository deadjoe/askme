...
from askme.retriever.base import RetrievalResult, Document
    ...
    async def rerank(
        ...
        try:
            ...
            for i in range(0, len(pairs), batch_size):
                ...
                logger.debug(f"Reranking batch {i//batch_size + 1}: {len(batch_pairs)} pairs")
            ...
            for i, (original_result, rerank_score) in enumerate(zip(original_docs, all_scores)):
...
class RerankingService:
    ...
    def __init__(self, config: RerankConfig, cohere_api_key: Optional[str] = None):
        ...
        self.cohere_reranker = CohereReranker(config, cohere_api_key) if config.cohere_enabled else None
    ...
    async def rerank(
        ...
        try:
            logger.debug(f"Using primary reranker: {primary_reranker.__class__.__name__}")
            ...
            if fallback_reranker:
                try:
                    logger.info(f"Falling back to: {fallback_reranker.__class__.__name__}")
