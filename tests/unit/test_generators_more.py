...
async def test_ollama_fallback_on_error(monkeypatch: Any) -> None:
    ...
    class _Client:
        async def post(self, *args: Any, **kwargs: Any):  # type: ignore[no-redef]
            raise RuntimeError("boom")

    async def _client_get(self) -> Any:  # type: ignore[no-redef]
        return _Client()
...
async def test_ollama_success(monkeypatch: Any) -> None:
    ...
    class _Resp:
        def raise_for_status(self):  # type: ignore[no-redef]
            return None
