...
async def test_hybrid_search_rrf_forces_fallback_when_native_unavailable():
    ...
    with patch("askme.retriever.milvus_retriever.HYBRID_SEARCH_AVAILABLE", False):
        ...
        out = await r.hybrid_search([0.1], {1: 0.5}, HybridSearchParams(use_rrf=True, rrf_k=10, topk=5))
...
async def test_update_document_success_path():
    ...
    doc = Document(id="u1", content="x", metadata={}, embedding=[0.1], sparse_embedding={0: 1.0})
